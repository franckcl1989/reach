...
| Duration presentation | first-party shared formatter (0.1.4) | The 0.1.4 output contract requires separated units (`5 s`, `16 ms`, `1.2 s`) shared by DNS, TCP, and ICMP. `humantime` was replaced by one small first-party formatter because its compact output (`5s`, `16ms`) contradicts the contract, and no mature formatter promised the exact separated-unit shape. |
...
| System resolver | macOS/Windows: `dns-lookup`; Linux static release: `gai-core` + first-party bounded formal DNS client (0.1.4) | `dns-lookup` retains the native OS path on macOS/Windows. Static musl cannot represent arbitrary host glibc NSS, so Linux uses `gai-core` for the mature NSS parser/simulator and `/etc/hosts` parser. A reached DNS source was originally served by `hickory-resolver` 0.26; 0.1.4 replaces it with a bounded first-party client built on `hickory-proto` and Tokio sockets because the 0.1.4 contract promises per-exchange observability (the exact endpoint, wire query name, transport, and timing of every formal exchange), and a library that hides which server and query it actually used cannot be used as proof of that behavior. The first-party client retains the observable semantics of the previous Hickory configuration — resolv.conf server order, `timeout`/`attempts`, search/domain/ndots expansion, A/AAAA independence, `inet6` family ordering, UDP-with-TCP-fallback-on-truncation, and answer order/duplicates — and reuses the audited `hickory-proto` codec plus the Direct DNS exchange implementation, so no new protocol codec is written. Reach glue only maps executed sources to Core and rejects a reached unsupported source/option. `gai-probe` 0.5.1 was evaluated but not selected: its DNS source discards search/domain/ndots, uses UDP-only server configuration, and cannot satisfy the formal-target contract. `tokio-system-resolver` 0.5.0 remains rejected because it does not compile for Windows. |
