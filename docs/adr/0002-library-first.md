...
| System resolver | macOS/Windows: `dns-lookup`; Linux static release: `gai-core` + `hickory-resolver` | `dns-lookup` retains the native OS path on macOS/Windows. Static musl cannot represent arbitrary host glibc NSS, so Linux uses `gai-core` for the mature NSS parser/simulator and `/etc/hosts` parser, then Hickory 0.26 for a reached DNS source with mature `/etc/resolv.conf`, search/domain/ndots, A/AAAA, timeout/attempt, and DNS transport behavior. Reach glue only maps the executed source to Core and rejects a reached unsupported source/option. `gai-probe` 0.5.1 was evaluated but not selected: its DNS source discards search/domain/ndots, uses UDP-only server configuration, and cannot satisfy the formal-target contract. `tokio-system-resolver` 0.5.0 remains rejected because it does not compile for Windows. |
